package club.laiyouxu.cas.custom.captcha.config;

import club.laiyouxu.cas.custom.captcha.CasCaptchaWebflowConfigurer;
import club.laiyouxu.cas.custom.captcha.ValidateCaptchaAction;
...
@ConditionalOnProperty(prefix = "scp.cas.recaptcha", name = "enabled", havingValue = "true", matchIfMissing = true)
@Slf4j
public class CasCaptchaConfiguration implements CasWebflowExecutionPlanConfigurer {
    ...
    public Action validateCaptchaAction() {
        return new ValidateCaptchaAction(casProperties.getRecaptcha());
    }
